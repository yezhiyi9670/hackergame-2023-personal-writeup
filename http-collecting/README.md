...
本题中，你可以向一个 nginx 服务器（对应的容器为**默认配置**下的 **`nginx:1.25.2-bookworm`**）发送 HTTP 请求。你需要获取到不同的 HTTP 响应状态码以获取 flag，其中：
...
> 网址：http://202.38.93.111:11461/

![](./assets/gui.png)

一个简单的用户界面，帮助选手编写并提交请求。下方会直接显示响应结果。这免去了选手使用某些专用工具提交请求的麻烦。
